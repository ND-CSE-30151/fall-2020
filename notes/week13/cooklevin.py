...
def make_phi(m, w, k):
    ...
    that is satisfiable iff m accepts w in at most k steps."""
    n = len(w)
    ...
    for j in range(n):
        phi_start &= x[1,j+3,w[j]]          # input symbols
    for j in range(n+3, k+4):
        phi_start &= x[1,j,syntax.BLANK]    # blank symbols
    phi_start &= x[1,k+4,"#"]               # endmarker
